...
public:
    ...
    Matrix_dense<T> get_conjugate();
    Matrix_dense<T> get_conjugate_transpose();
...
template <typename T>
Matrix_dense<T> Matrix_dense<T>::get_conjugate()
{
    static_assert(std::is_same<T, std::complex<typename T::value_type>>::value,
                  "Valid for complex matrices only");

    Matrix_dense<T> conjugate_matrix(_rows, _cols);
    for (int i = 0; i < _rows; i++)
    {
        for (int j = 0; j < _cols; j++)
        {
            conjugate_matrix._matrix[i][j] = conj(_matrix[i][j]);
        }
    }
...
template <typename T>
void Matrix_dense<T>::conjugate()
{
    *this = get_conjugate();
}

template <typename T>
Matrix_dense<T> Matrix_dense<T>::get_conjugate_transpose()
{

    static_assert(std::is_same<T, std::complex<typename T::value_type>>::value,
                  "Valid for complex matrices only");

    Matrix_dense<T> conjugate_matrix_T(_cols, _rows);
    for (int j = 0; j < _cols; j++)
    {
        for (int i = 0; i < _rows; i++)
        {
            conjugate_matrix_T._matrix[j][i] = conj(_matrix[i][j]);
        }
    }
    return conjugate_matrix_T;
}

template <typename T>
void Matrix_dense<T>::conjugate_transpose()
{
    *this = get_conjugate_transpose();
}
...
template <typename T>
const int Matrix_dense<T>::nrows() const
{
    return _rows;
}

template <typename T>
const int Matrix_dense<T>::ncols() const
{
    return _cols;
}
